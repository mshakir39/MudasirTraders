...
async function migrateInvoiceVoidFields() {
  ...
  try {
    ...
    if (invoicesWithoutStatus === 0) {
      console.log(
        '✅ All invoices already have status field. Migration not needed.'
      );
      return;
    }
    ...
    const postMigrationStats = await invoiceCollection
      .aggregate([
        {
          $group: {
            _id: '$status',
    ...
    if (stillMissingStatus > 0) {
      console.log(
        `   ❌ ERROR: ${stillMissingStatus} invoices still missing status field`
      );
    ...
    const sampleInvoices = await invoiceCollection.find({}).limit(3).toArray();
