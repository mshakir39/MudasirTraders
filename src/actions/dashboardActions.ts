...
export async function getDashboardStats() {
  try {
    ...
    const totalSales = Array.isArray(sales) ? sales.reduce((sum: number, sale: any) => sum + (sale.totalAmount || 0), 0) : 0;
    ...
    const totalStockItems = Array.isArray(stock) ? stock.reduce((sum: number, item: any) => {
      if (item.seriesStock && Array.isArray(item.seriesStock)) {
        return sum + item.seriesStock.reduce((itemSum: number, series: any) => itemSum + (parseInt(series.inStock) || 0), 0);
      }
      return sum;
    }, 0) : 0;
    ...
    const recentSales = Array.isArray(sales) ? sales.filter((sale: any) => 
      new Date(sale.saleDate) >= sevenDaysAgo
    ) : [];
    
    // Get top selling products
    const productSales = Array.isArray(sales) ? sales.reduce((acc: any, sale: any) => {
      const key = `${sale.brandName}-${sale.series}`;
      if (!acc[key]) {
        acc[key] = { brandName: sale.brandName, series: sale.series, totalQuantity: 0, totalRevenue: 0 };
      }
...
export async function getSalesTrend(startDate: Date, endDate: Date) {
  try {
    ...
    // Group sales by date
    const salesByDate = Array.isArray(sales) ? sales.reduce((acc: any, sale: any) => {
    ...
    const salesTrend = Object.values(salesByDate).sort((a: any, b: any) => 
...
export async function getInventoryByBrand() {
  try {
    ...
    const inventoryByBrand = Array.isArray(stock) ? stock.map((item: any) => {
      const totalStock = item.seriesStock && Array.isArray(item.seriesStock) 
        ? item.seriesStock.reduce((sum: number, series: any) => sum + (parseInt(series.inStock) || 0), 0)
        : 0;
