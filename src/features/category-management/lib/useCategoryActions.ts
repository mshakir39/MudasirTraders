...
import { CategoryWithBatteryData, CategoryFormData } from '@/features/category-management/entities/category/model/types';
...
export const useCategoryActions = ({
  ...
  const updateCategory = useCallback(async (categoryId: string, data: Partial<CategoryFormData>) => {
    try {
  ...
  const updateBatteryPrice = useCallback(async (categoryId: string, batteryId: string, price: number) => {
    try {
      const response = await fetch(`/api/categories/${categoryId}/battery/${batteryId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const deleteBattery = useCallback(async (categoryId: string, batteryId: string) => {
  ...
  const updateGlobalSalesTax = useCallback(async (categoryId: string, tax: number) => {
    try {
      const response = await fetch(`/api/categories/${categoryId}/sales-tax`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
