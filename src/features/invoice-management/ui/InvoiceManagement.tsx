...
import { Invoice, InvoiceFilter, InvoiceModalState, InvoiceModalType } from '@/entities/invoice';
...
import { InvoiceFilters, InvoiceDataGrid, InvoiceModals, InvoiceDeleteModal } from './components';
import { categoriesAtom, stockAtom, invoicesAtom, fetchInvoicesAtom, fetchStockAtom, setInvoicesAtom, setStockAtom } from '@/store/sharedAtoms';
...
export const InvoiceManagement: React.FC<InvoiceManagementProps> = ({
  ...
  const isDataReady = categories && stock && globalInvoices && 
                     Array.isArray(categories) && Array.isArray(stock) && Array.isArray(globalInvoices) &&
                     categories.length >= 0 && stock.length >= 0 && globalInvoices.length >= 0;
  ...
  const transformedInitialInvoices = (initialInvoices || []).map((invoice: any) => ({
    ...invoice,
  ...
  const [invoices, setInvoices] = useState<Invoice[]>(globalInvoices.length > 0 ? globalInvoices : transformedInitialInvoices);
  ...
  const handleOpenModal = useCallback((type: InvoiceModalType, data?: Invoice) => {
    setModalState({
      isOpen: true,
      type,
      data
  ...
  const handleCreateInvoice = useCallback(async (invoiceData: Partial<Invoice>) => {
    try {
      setLoading(true);
      ...
      const lastInvoiceNo = invoices.length > 0 ? invoices[0].invoiceNo : undefined;
      const invoiceNo = invoiceData.invoiceNo || InvoiceApi.generateInvoiceNumber(lastInvoiceNo);
      
      // Calculate due date if not provided
      const dueDate = invoiceData.dueDate || InvoiceApi.calculateDueDate(new Date());
      ...
      const newInvoice: Invoice = {
        ...
        createdDate: invoiceData.useCustomDate && invoiceData.customDate 
  ...
  const handleUpdateInvoice = useCallback(async (invoiceData: Partial<Invoice>) => {
    try {
      setLoading(true);
  ...
  const handleAddPayment = useCallback(async (invoiceId: string, paymentAmount: number, paymentMethod: string) => {
    try {
      setLoading(true);
      ...
      setInvoices(prev => prev.map(invoice => {
        if (invoice.id === invoiceId) {
          const newReceivedAmount = result.receivedAmount || (invoice.receivedAmount || 0) + paymentAmount;
          const batteryRate = invoice.batteriesRate || 0;
          const additionalPayments = (result.additionalPayment || invoice.additionalPayment || []).reduce((sum: number, payment: any) => sum + (payment.amount || 0), 0);
          const newRemainingAmount = result.remainingAmount !== undefined 
            ? result.remainingAmount 
            : Math.max(0, (invoice.totalAmount || 0) - newReceivedAmount - batteryRate - additionalPayments);
          
          return {
            ...invoice,
            receivedAmount: newReceivedAmount,
            remainingAmount: newRemainingAmount,
            paymentStatus: result.paymentStatus || (newRemainingAmount <= 0 ? 'paid' : newReceivedAmount > 0 ? 'partial' : 'pending'),
            updatedAt: new Date(),
            // Update additionalPayment array if returned from API
            additionalPayment: result.additionalPayment || invoice.additionalPayment
          };
        }
        return invoice;
      }));
  ...
  useEffect(() => {
    const handleConsolidationRefresh = async (event: Event) => {
      ...
      console.log('🔄 Received consolidation refresh event:', customEvent.detail);
    ...
    // Add event listener
    window.addEventListener('consolidation-refresh', handleConsolidationRefresh);
    
    // Cleanup event listener
    return () => {
      window.removeEventListener('consolidation-refresh', handleConsolidationRefresh);
    };
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <InvoiceDataGrid
        ...
        onPreviewReplacement={(replacementInvoiceId) => {
          ...
          const replacementInvoice = invoices.find(inv => inv.id === replacementInvoiceId);
