  ...
  const handleCustomerNameChange = (e: {
    ...
    if (customerInfo) {
      ...
      setInvoiceData({
        ...
        customerAddress: customerInfo.address || invoiceData.customerAddress || '',
  ...
  {['WalkIn Customer', 'Regular Customer'].map((type) => (
    <label key={type} className="flex items-center cursor-pointer">
      ...
      <div className="relative w-4 h-4">
        <div
          className={`w-4 h-4 rounded-full border-2 transition-all duration-200 flex items-center justify-center ${
            invoiceData.customerType === type ? 'border-blue-600' : 'border-gray-300'
          }`}
          style={{
            background: invoiceData.customerType === type
              ? 'linear-gradient(135deg, rgb(30, 58, 138), rgb(29, 78, 216), rgb(37, 99, 235))'
              : 'transparent',
            boxShadow: invoiceData.customerType === type
      ...
      <span className="text-sm ml-2 text-gray-700">
        {type === 'WalkIn Customer' ? 'Walk-in Customer' : 'Regular Customer'}
      </span>
    </label>
  ))}
</div>
      ...
      {invoiceData.customerType === 'WalkIn Customer' ? (
        ...
        <div>
          ...
          <input
            ...
            required
            minLength={1}
            readOnly={invoiceData.customerType === 'Regular Customer' && !!invoiceData.clientId}
            placeholder={
              invoiceData.customerType === 'Regular Customer' && !invoiceData.clientId
