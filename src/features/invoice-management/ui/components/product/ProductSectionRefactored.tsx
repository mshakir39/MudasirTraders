...
const ProductSectionRefactored: React.FC<ProductSectionRefactoredProps> = ({
  ...
  const { calculateTotalPrice, calculateWarrantyEndDate, formatPrice } = useProductCalculations();
    ...
    return (
      <div className="space-y-4">
        ...
        <ProductPriceInput
          ...
          onQuantityChange={(quantity) => handleFieldChange('quantity', quantity)}
        />

        {/* Warranty Toggle */}
        <ProductWarrantyToggle
          noWarranty={accordionDataItem.noWarranty}
          onWarrantyChange={(noWarranty: boolean) => handleFieldChange('noWarranty', noWarranty)}
        />
        ...
        {!accordionDataItem.noWarranty && (() => {
          ...
          <ProductWarrantyFields
            ...
            onStartDateChange={(date) => handleFieldChange('warrentyStartDate', date)}
            onDurationChange={(duration) => handleFieldChange('warrentyDuration', duration)}
  ...
  return (
    <div className="space-y-4">
      {accordionData && Object.keys(accordionData).length > 0 ? (
        ...
        const brandText = accordionDataItem.brandName ? `${accordionDataItem.brandName}` : '';
        const seriesText = accordionDataItem.series ? ` - ${accordionDataItem.series}` : '';
        const hasBothSelected = accordionDataItem.brandName && accordionDataItem.series;
        ...
        return (
          <Accordion
            ...
            addOnClick={() => accordionMethods.handleAddAccordion(accordionIndex)}
            removeOnClick={() => accordionMethods.handleRemoveAccordion(accordionIndex)}
