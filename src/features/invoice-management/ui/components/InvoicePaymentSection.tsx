...
export const InvoicePaymentSection: React.FC<InvoicePaymentSectionProps> = ({
  ...
  const effectiveTotalAmount = invoiceData.paymentMethod?.includes('Old Battery') 
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3">
          {['Cash', 'Bank Transfer', 'JazzCash', 'EasyPaisa', 'Old Battery', 'Pay Later', 'Other'].map(method => (
      ...
      {invoiceData.paymentMethod?.includes('Pay Later') && (
        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3">
          <p className="text-sm text-yellow-800">
            ...
            the remaining balance can be paid later using the "Add Payment" button.
          </p>
        </div>
      )}
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <div className="grid grid-cols-1 gap-2 text-sm">
          ...
          {invoiceData.paymentMethod?.includes('Old Battery') && batteriesRate > 0 && (
            <div className="flex justify-between">
              <span className="font-medium">Battery Amount:</span>
              <span className="font-bold text-blue-600">-Rs {batteriesRate.toLocaleString()}</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="font-medium">Effective Total:</span>
            <span className="font-bold">Rs {effectiveTotalAmount.toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
          ...
          {previousRemainingAmount > 0 && (
            <div className="flex justify-between">
              <span className="font-medium text-orange-600">Previous Remaining:</span>
              <span className="font-bold text-orange-600">Rs {previousRemainingAmount.toLocaleString()}</span>
            </div>
          )}
          <div className="flex justify-between pt-2 border-t">
            <span className="font-medium text-red-600">Remaining:</span>
            <span className="font-bold text-red-600">Rs {(remainingAmount + previousRemainingAmount).toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
