...
export async function PATCH(req: any, res: any) {
  try {
    ...
    if (action === 'cleanupStockData') {
      ...
      for (const stockDoc of allStock) {
        if (stockDoc.seriesStock && Array.isArray(stockDoc.seriesStock)) {
          ...
          const updatedSeriesStock = stockDoc.seriesStock.map((series: any) => {
            ...
            if (originalInStock !== newInStock || originalSoldCount !== newSoldCount || originalProductCost !== newProductCost) {
              needsUpdate = true;
              console.log(`🔄 Converting ${series.series}: inStock "${originalInStock}" → ${newInStock}, soldCount "${originalSoldCount}" → ${newSoldCount}, productCost "${originalProductCost}" → ${newProductCost}`);
            }
      ...
      console.log(`✅ Stock data cleanup completed. Updated ${updatedCount} documents.`);
