...
export async function GET(
  ...
  try {
    ...
    if (ObjectId.isValid(params.customerId) && params.customerId.length === 24) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          return Response.json({ error: 'Database operation not supported' }, { status: 500 });
        }
      }
    }

    console.log('📊 Total invoices found:', Array.isArray(allInvoices) ? allInvoices.length : 'Not an array');
