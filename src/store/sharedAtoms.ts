...
export const fetchCategoriesAtom = atom(
  ...
  async (get, set) => {
    ...
    try {
      ...
      set(categoriesAtom, result.success && Array.isArray(result.data) ? result.data : []);
    } catch (error) {
...
export const fetchBrandsAtom = atom(
  ...
  async (get, set) => {
    ...
    try {
      ...
      const result = await response.json();
      set(brandsAtom, result.success && Array.isArray(result.data) ? result.data : []);
    } catch (error) {
...
export const setBrandsAtom = atom(
  null,
  (get, set, brands: any[]) => {
...
export const fetchCustomersAtom = atom(
  null,
  async (get, set) => {
    ...
    try {
      ...
      const result = await response.json();
      set(customersAtom, result.success && Array.isArray(result.data) ? result.data : []);
    } catch (error) {
...
export const setCustomersAtom = atom(
  null,
  (get, set, customers: any[]) => {
    set(customersAtom, customers);
  }
);

// Action atoms for sales
export const fetchSalesAtom = atom(
  null,
  async (get, set) => {
...
export const setSalesAtom = atom(
  null,
  (get, set, sales: any[]) => {
    set(salesAtom, sales);
  }
...
export const fetchStockAtom = atom(
  null,
  async (get, set) => {
    ...
    try {
      ...
      const result = await response.json();
      const stockData = result.success && Array.isArray(result.data) ? result.data : [];
...
export const setStockAtom = atom(
  null,
  (get, set, stock: any[]) => {
    set(stockAtom, stock);
  }
...
export const fetchInvoicesAtom = atom(
  ...
  async (get, set) => {
    ...
    try {
        ...
        const transformedInvoices = result.data
          .map((invoice: any) => {
            ...
            const calculateTotalAmount = () => {
              ...
              if (invoice.products && Array.isArray(invoice.products)) {
                return invoice.products.reduce((sum: number, product: any) => {
                  return sum + (typeof product.totalPrice === 'number' ? product.totalPrice : 0);
                }, 0);
              }
              
              // Use remainingAmount + receivedAmount as fallback
              const remaining = typeof invoice.remainingAmount === 'number' ? invoice.remainingAmount : 0;
              const received = typeof invoice.receivedAmount === 'number' ? invoice.receivedAmount : 0;
            ...
            return {
              ...
              customerContactNumber: invoice.customerContactNumber || invoice.customerPhone || '',
              ...
              remainingAmount: invoice.remainingAmount || (totalAmount - (invoice.receivedAmount || 0)),
              paymentMethod: Array.isArray(invoice.paymentMethod) ? invoice.paymentMethod : (invoice.paymentMethod ? [invoice.paymentMethod] : []),
              paymentStatus: invoice.paymentStatus || 'pending',
              status: invoice.status || 'active',
              useCustomDate: invoice.useCustomDate || false,
              customDate: invoice.customDate || '',
              createdDate: new Date(invoice.createdDate || invoice.createdAt || Date.now()),
              ...
              consolidatedInvoiceNumbers: invoice.consolidatedInvoiceNumbers || undefined,
      ...
      set(invoicesErrorAtom, error instanceof Error ? error.message : 'Failed to fetch invoices');
      set(invoicesAtom, []);
    } finally {
...
export const setInvoicesAtom = atom(
  null,
  (get, set, invoices: any[]) => {
