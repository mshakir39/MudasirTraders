...
interface AuthStatusProps {
  onStatusChange?: (status: { isSignedIn: boolean; isDashboardUnlocked: boolean }) => void;
  children?: React.ReactNode;
}

export default function AuthStatus({ onStatusChange, children }: AuthStatusProps) {
  const { status, data: session } = useSession();
  const [isDashboardUnlocked, setIsDashboardUnlocked] = useState(false);
  const [isMounted, setIsMounted] = useState(false);
  const [lastNotifiedValues, setLastNotifiedValues] = useState<{isSignedIn: boolean, isDashboardUnlocked: boolean} | null>(null);
  ...
  useEffect(() => {
    if (isMounted && onStatusChange) {
      ...
      const hasChanged = !lastNotifiedValues || 
