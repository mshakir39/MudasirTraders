...
interface CustomerNameAutocompleteProps {
  value: string;
  onChange: (e: { target: { name: string; value: string; customerInfo?: CustomerInfo } }) => void;
  name: string;
  label: string;
  placeholder?: string;
  required?: boolean;
        ...
        if (Array.isArray(invoices)) {
          ...
          invoices.forEach((invoice: any) => {
            ...
            if (customerName && customerName.trim() !== '' && customerName !== '-') {
              ...
              const uniqueKey = contactNumber && contactNumber.trim() !== '' ? contactNumber : customerName;
              
              // Use the most recent invoice data for each customer
              if (!customerMap.has(uniqueKey) || new Date(invoice.createdDate) > new Date(customerMap.get(uniqueKey)?.name || '')) {
